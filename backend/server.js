...
app.post('/mechanic/job/start', auth('mechanic'), async (req,res) => {
  ...
  try {
    const result = await pool.query('UPDATE jobs SET status=$1, start_time=NOW() WHERE id=$2 RETURNING *',
      ['in_progress', job_id]);
    res.json(result.rows[0]);
  } catch(err) {
    res.status(500).json({ error: err.message });
...
app.post('/mechanic/job/complete', auth('mechanic'), async (req,res) => {
  ...
  try {
    const job = await pool.query('UPDATE jobs SET status=$1, parts_used=$2, end_time=NOW() WHERE id=$3 RETURNING *',
      ['completed', parts_used, job_id]);
